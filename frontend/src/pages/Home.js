...
import { FiPlay, FiInfo, FiChevronRight, FiTrendingUp, FiStar, FiDownload } from 'react-icons/fi';
...
const Home = () => {
  ...
  return (
    <div className="home page-enter">
      {/* Hero Section */}
      {hero && (
        <section className="hero" style={{ '--hero-bg': `url(${heroPosterUrl})` }}>
          <div className="hero-backdrop" />
          <div className="container hero-content">
            <div className="hero-text">
              <div className="hero-badges">
                <span className="badge badge-accent">{hero.quality}</span>
                {hero.genre?.slice(0,2).map(g => <span key={g} className="badge badge-dark">{g}</span>)}
              </div>
              <h1 className="hero-title">{hero.title}</h1>
              {hero.titleBn && <p className="hero-title-bn">{hero.titleBn}</p>}
              <div className="hero-meta">
                <span><FiStar className="gold" /> {hero.averageRating > 0 ? hero.averageRating.toFixed(1) : 'New'}</span>
                <span>{hero.releaseYear}</span>
                {hero.duration && <span>{Math.floor(hero.duration / 60)}h {hero.duration % 60}m</span>}
                <span>{hero.language}</span>
              </div>
              <p className="hero-desc">{hero.description?.slice(0, 200)}...</p>
              <div className="hero-actions">
                <Link to={`/movies/${hero._id}`} className="btn btn-primary btn-lg">
                  <FiPlay /> Watch Now
                </Link>
                <Link to={`/movies/${hero._id}`} className="btn btn-ghost btn-lg">
                  <FiInfo /> More Info
                </Link>
              </div>
            </div>
          ...
          {featured.length > 1 && (
            <div className="hero-dots">
              {featured.map((_, i) => (
                <button key={i} className={`dot-btn ${i === heroIndex ? 'active' : ''}`} onClick={() => setHeroIndex(i)} />
              ))}
            </div>
          )}
        </section>
      ...
      <section className="movies-section">
        <div className="container">
          <div className="section-header">
            <h2 className="section-title"><FiTrendingUp /> Latest Movies</h2>
          ...
          <div className="browse-more">
            <Link to="/movies" className="btn btn-outline btn-lg">Browse All Movies</Link>
          </div>
        </div>
      </section>
      ...
      <section className="genre-section">
        <div className="container">
          ...
          <div className="genre-grid">
            {GENRES.map((genre, i) => (
              <Link key={genre} to={`/movies?genre=${genre}`} className="genre-card" style={{ '--delay': `${i * 0.05}s` }}>
      ...
      <section className="stats-banner">
        <div className="container stats-grid">
          <div className="stat-item">
            ...
            <span>All movies available for download</span>
          </div>
          <div className="stat-item">
